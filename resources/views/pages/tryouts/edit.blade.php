    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            ...
            <div class="intro-y box">
                ...
                @endif
                {!! Form::model($tryout, ['method' => 'PATCH','enctype'=>'multipart/form-data','route' => ['tryouts.update', $tryout->id]]) !!}
                @csrf
                <div id="input" class="p-5">
                    <div class="preview">
                        <div>
                            <label for="regular-form-1" class="form-label">Name</label>
                            {!! Form::text('name', $tryout->name, array('placeholder' => 'Name','class' => 'form-control','required')) !!}
                        </div>
                        <div class="mt-3">
                            <label for="regular-form-2" class="form-label">Category</label>
                            <select name="category" class="form-control">
                                @foreach ($categories as $key => $cat)
                                    @if($tryout->category_id == $cat->id)
                        ...
                        <div class="form-check form-switch flex flex-col items-start mt-3">
                            ...
                            <input name="publish" id="post-form-5" class="form-check-input" type="checkbox" @if($tryout->publish == 'on'){ {{ 'checked' }} } @endif>
                        </div>
